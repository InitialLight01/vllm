    ...
    if has_prefill:
        ...
        for chunk in prefill_metadata.chunks:
            ...
            if current_platform.is_xpu():
                ...
                # SM80/SM121 Triton fallback (no DeepGEMM). FP4 Q/KV is
                # rejected earlier; only per-tensor FP8 reaches here.
...
class SparseAttnIndexer(CustomOp):
    ...
    def __init__(
        ...
        if current_platform.is_cuda() and not is_deep_gemm_supported():
            # On SM80/SM121 (A100, GB10) DeepGEMM is unavailable — fall back
            # to the Triton sparse-MLA path. is_deep_gemm_supported() encodes
            # the SM-arch + has_deep_gemm() gate; if not supported, downgrade
            # the hard error from upstream to a one-time warning so the
            # indexer routes through the Triton kernels in
            # `mqa_logits_triton.py`.
            logger.warning_once(
