...
def compress_norm_rope_store_triton(
    ...
    if head_dim == 512:
        ...
        num_warps = 4
        # Outlier clip: the SM80 EMULATION pipeline uses ±5; the SM12x
        # validated path has no clip (0 disables it).
        clip_bound = 5.0 if current_platform.is_sm80_context() else 0.0
    elif use_fp4_cache:
...
def _fused_kv_compress_norm_rope_insert_sparse_attn(
    ...
    CLIP_BOUND: tl.constexpr,  # outlier clip: 5.0 (SM80 EMULATION) / 0 = disabled (SM12x)
):
    ...
    # --- outlier suppression (SM80 EMULATION only) ---
    ...
    # negligible signal). CLIP_BOUND == 0 disables clipping (SM12x validated
    # path has no clip at all).
    if CLIP_BOUND > 0:
